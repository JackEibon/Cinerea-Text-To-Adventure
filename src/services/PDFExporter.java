...
public class PDFExporter {
	...
	public void exportConfigs(List<Config> configs, File file) throws IOException {
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			for (int i = 0; i < 2; i++) {
				Cell[] headerFooter = new Cell[] { createHeaderFooterCell("#"), createHeaderFooterCell("User ID"),
						createHeaderFooterCell("Setting Key"), createHeaderFooterCell("Setting Value") };
				for (Cell celda : headerFooter) {
					if (i == 0)
						table.addHeaderCell(celda);
					else
			...
			for (Config c : configs) {
				...
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(c.getIdUser()))));
	...
	public void exportEntityStatuses(List<EntityStatus> statuses, File file) throws IOException {
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			addLogo(doc);
			doc.add(new Paragraph("Entity Statuses Report").setBold().setFontSize(12)
					.setTextAlignment(TextAlignment.CENTER));
			doc.add(new Paragraph("").setMarginTop(30));
			...
			for (int i = 0; i < 2; i++) {
				Cell[] headerFooter = new Cell[] { createHeaderFooterCell("#"), createHeaderFooterCell("User ID"),
						createHeaderFooterCell("NPC ID"), createHeaderFooterCell("Status Effect ID") };
				for (Cell celda : headerFooter) {
					if (i == 0)
						table.addHeaderCell(celda);
					else
			...
			for (EntityStatus es : statuses) {
				table.addCell(
						new Cell().setTextAlignment(TextAlignment.CENTER).add(new Paragraph(String.valueOf(indx))));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(es.getIdUser() != null ? String.valueOf(es.getIdUser()) : "N/A")));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(es.getIdNpc() != null ? String.valueOf(es.getIdNpc()) : "N/A")));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(es.getIdStatusEffect()))));
				indx++;
			}
			doc.add(table);
		}
		...
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			for (int i = 0; i < 2; i++) {
				Cell[] headerFooter = new Cell[] { createHeaderFooterCell("#"), createHeaderFooterCell("Name"),
						createHeaderFooterCell("Type"), createHeaderFooterCell("Description") };
				for (Cell celda : headerFooter) {
					if (i == 0)
						table.addHeaderCell(celda);
					else
	...
	public void exportNpcs(List<Npc> npcs, File file) throws IOException {
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			for (int i = 0; i < 2; i++) {
				Cell[] headerFooter = new Cell[] { createHeaderFooterCell("#"), createHeaderFooterCell("Name"),
						createHeaderFooterCell("Role"), createHeaderFooterCell("Location") };
				for (Cell celda : headerFooter) {
					if (i == 0)
						table.addHeaderCell(celda);
					else
	...
	public void exportScores(List<Score> scores, File file) throws IOException {
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			for (int i = 0; i < 2; i++) {
				Cell[] headerFooter = new Cell[] { createHeaderFooterCell("#"), createHeaderFooterCell("User ID"),
						createHeaderFooterCell("Best Score"), createHeaderFooterCell("Last Score") };
				for (Cell celda : headerFooter) {
					if (i == 0)
						table.addHeaderCell(celda);
					else
			...
			for (Score s : scores) {
				table.addCell(
						new Cell().setTextAlignment(TextAlignment.CENTER).add(new Paragraph(String.valueOf(indx))));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(s.getIdUser()))));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(s.getBestScore()))));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(s.getLastScore()))));
				indx++;
			}
			doc.add(table);
		}
	...
	public void exportStatuses(List<Status> statuses, File file) throws IOException {
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			addLogo(doc);
			doc.add(new Paragraph("Status Effects Report").setBold().setFontSize(12)
					.setTextAlignment(TextAlignment.CENTER));
			doc.add(new Paragraph("").setMarginTop(30));
			...
			for (int i = 0; i < 2; i++) {
				Cell[] headerFooter = new Cell[] { createHeaderFooterCell("#"), createHeaderFooterCell("Effect Name"),
			...
			for (Status s : statuses) {
				...
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(s.getDuration()))));
				indx++;
			}
			doc.add(table);
		}
	...
	public void exportUserInventories(List<UserInventory> inventories, File file) throws IOException {
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			addLogo(doc);
			doc.add(new Paragraph("User Inventories Report").setBold().setFontSize(12)
					.setTextAlignment(TextAlignment.CENTER));
			doc.add(new Paragraph("").setMarginTop(30));
			...
			for (int i = 0; i < 2; i++) {
				Cell[] headerFooter = new Cell[] { createHeaderFooterCell("#"), createHeaderFooterCell("User ID"),
						createHeaderFooterCell("Item ID"), createHeaderFooterCell("Quantity") };
				for (Cell celda : headerFooter) {
					if (i == 0)
						table.addHeaderCell(celda);
					else
			...
			for (UserInventory ui : inventories) {
				table.addCell(
						new Cell().setTextAlignment(TextAlignment.CENTER).add(new Paragraph(String.valueOf(indx))));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(ui.getIdUser()))));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(ui.getIdItem()))));
				table.addCell(new Cell().setTextAlignment(TextAlignment.CENTER)
						.add(new Paragraph(String.valueOf(ui.getQuantity()))));
				indx++;
			}
			doc.add(table);
		}
