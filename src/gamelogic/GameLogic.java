  ...
  //  public static GameCanvas canvas= new GameCanvas();
		...
		t += "\n===========GAME OVER==========\n";
            ...
            for (Item ite : player.getInventory()) {
            	for (String tag : ite.getTags()) {
            		score+=10;
            		if (tag.equalsIgnoreCase("gem")) score+=500;
					
				}
				
			}
            //else if (!w.isAlive()) score += 500;
            if (player.isAlive()) score += 356;
            ...
            t+="Credits:\n"
                ...
                if (countingScore < score && (minitick % 23 == 0)) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score - 50) && minitick % 5 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score - 200) && minitick % 3 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score - 1000)) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score / 2) && minitick % 7 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score / 3) && minitick % 5 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score / 4) && minitick % 3 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score / 5) && minitick % 2 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score / 6)) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score - 10) && minitick % 2 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score - 42) && minitick % 3 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score - 100) && minitick % 11 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score - 500)) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score / 2) && minitick % 7 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score / 3) && minitick % 5 == 0) countingScore++;
                ...
                if (countingScore < (score / 5) && minitick % 2 == 0) countingScore++;
                player.setCountingScore(countingScore);
                if (countingScore < (score / 2)) countingScore++;
                player.setCountingScore(countingScore);
	...
	public void clock() {
		...
		if (minitick >= 100) {
         ...
         nightEnd++; }
         switch (tock) {
                        case 1: night = false; break;          // morning
                        case 4: night = true; dayEnd++; break;
                        case 6: nigthHP=670;break;
                        // night
	 ...
	 * public void clock() {
        if (framecount >= 99) {
            tickNow = tick;
            tick++;
            //GameWindow.logPanel.addMessage("tick:"+tick);
            tickNowFun(p1);
            tickNowFun(w);

            Node current = Cinerea.world.corner;
            for (int i = 0; i < 30; i++) {
                current = Cinerea.world.corner;
                for (int r = 0; r < i; r++) {
     <               if (current.s != null) { current = current.getS(); }
                }
            }
            ...
            if (tick >= 10) {
                ...
                if (tack >= 3) {
                    tack = 0;
                    tock++;
                    if (tock == 7) { tock = 1; nightEnd++; }
                    switch (tock) {
                        case 1: night = false; break;          // morning
                        case 4: night = true; dayEnd++; break; // night
                        case 6: break;                         // epitome
                    }
                }
            }
        }
