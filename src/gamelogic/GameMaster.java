...
10:
    private void updateMenu() {
        im betting on making it work with keys Pressed. I know
        it can work, but i am betting time.
        
        switch(GameWindow.getKeyPress()){
            case 30:{
    ...
    if (GameWindow.getKeyPresseds<KeyEvent) {
        selectedOption = Math.max(0, selectedOption - 1);
    }
    if (GameWindow.keyPresseds.containsKey(KeyEvent.VK_DOWN)) {
        selectedOption = Math.min(menuOptions.length - 1, selectedOption + 1);
    }
    if (GameWindow.keyPresseds.containsKey(KeyEvent.VK_ENTER)) {
...
private void drawMenu(Graphics g) {
    ...
    for (int i = 0; i < menuOptions.length; i++) {
        ...
        drawSpriteText(g, prefix + menuOptions[i], 100, y);
        y += 40;
    }
}
*/
